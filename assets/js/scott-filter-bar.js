 ...
 * <scott-filter-bar>
 * ------------------------------------------------------------------
 * A category filter bar for card grids (Field Notes, Insights, and
 * anywhere else a set of cards needs to be narrowed down).
 *
 * Each pill carries a small bar-chart indicator sized to how many
 * items are actually in that category - a nod to the bar-chart cut
 * into the "S" mark, and a way to make the indicator mean something
 * instead of just decorate the pill.
 *
 * USAGE
 * -----
 * <scott-filter-bar target="#fn-grid" item-selector=".fn-card-link">
 *   <script type="application/json">
 *     [
 *       { "key": "measurement", "label": "Measurement & Data", "color": "#2563eb" },
 *       { "key": "frontend",    "label": "Frontend & CSS", "color": "#0f766e" },
 *       { "key": "infra",       "label": "Infrastructure & Ops", "color": "#7c2d92" },
 *       { "key": "performance", "label": "Performance", "color": "#ca8a04" }
 *     ]
 *   </script>
 * </scott-filter-bar>
 *
 * Each item inside the target container needs a data-categories
 * attribute listing one or more category keys, space separated:
 *
 *   <a class="fn-card-link" data-categories="frontend performance" ...>
 *
 * If a card belongs to more than one category, list both keys - the
 * card will show up whichever one of those pills is active. Only one
 * pill can be active at a time - picking a new one replaces the last.
 *
 * ATTRIBUTES
 * ----------
 * target          (required) Selector for the grid/container holding the items.
 * item-selector   Selector for filterable items within target. Default: "[data-categories]"
 * category-attr   Attribute holding an item's categories. Default: "data-categories"
 * all-label       Label for the reset/all pill. Default: "All"
 *
 * If no child <script type="application/json"> category list is
 * given, categories are discovered automatically from the items
 * themselves and labeled by title-casing the key. Explicit labels
 * read better, so the JSON block is recommended. An optional "color"
 * per entry gives that pill its own accent (falls back to navy when
 * omitted) - see THEMING below.
 *
 * REQUIRED GLOBAL CSS (add once, wherever your item selector applies)
 * ---------------------------------------------------------------------
 *   .fn-card-link { transition: opacity .18s ease, transform .18s ease; }
 *   .fn-card-link.ff-hide { display: none; }
 *   .fn-card-link.ff-leaving,
 *   .fn-card-link.ff-entering { opacity: 0; transform: translateY(4px) scale(.98); }
 *
 * Swap ".fn-card-link" for whatever item-selector you're using on a
 * given page (insights cards, for instance). This lives in your
 * global stylesheet because the items being filtered are outside
 * this component's shadow root - the pill bar itself needs no
 * external CSS at all.
 *
 * EVENTS
 * ------
 * "scott-filter-change" - bubbles, composed.
 *   detail: { active: string[], visible: number, total: number }
 */
(function () {
  'use strict';
  ...
  var TEMPLATE_STYLES =
    ...
    '  white-space: nowrap;' +
    '}' +
    // Hover and active read from --pill-color (set per-pill inline from
    // the JSON config's "color" field) so each category gets its own
    // accent instead of one navy throughout. Falls back to --ff-navy
    // when a pill has no color (e.g. the "All" pill).
    ...
    _bind() {
      ...
      this._pills.forEach(function (pill) {
        pill.addEventListener('click', function () {
          ...
          // Single-select: picking a filter replaces whatever was active,
          // it doesn't add to it. Clicking the already-active pill again
          // (or "All") clears back to showing everything.
